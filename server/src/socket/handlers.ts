...
export function registerHandlers(io: Server, socket: Socket, deps: HandlerDeps): void {
  ...
  socket.on(CLIENT_EVENTS.submitCaption, ({ meme }: { meme: unknown }) => {
    ...
    // meme is passed through exactly as received — validation lives entirely
    // inside Room.submitCaption. The author is always socket.data.playerId;
    // the payload carries no identity field a client could forge.
    const result = room.submitCaption(data.playerId, meme);
    if (!result.ok) {
      emitError(socket, { code: result.error, messageHe: HEBREW_ERRORS[result.error] });
      return;
    }
  ...
  socket.on(CLIENT_EVENTS.swapPhoto, () => {
    if (!data.playerId || !data.roomCode) {
      emitError(socket, { code: "NOT_IN_ROOM", messageHe: HEBREW_ERRORS.NOT_IN_ROOM });
      return;
    }
    ...
    // No payload to destructure — the player to swap is always
    // socket.data.playerId (T-04-01), matching every other mutating
    // handler's identity rule.
    const result = room.swapPhoto(data.playerId);
    if (!result.ok) {
      emitError(socket, { code: result.error, messageHe: HEBREW_ERRORS[result.error] });
      return;
    }
  ...
  socket.on(CLIENT_EVENTS.skipRound, () => {
    if (!data.playerId || !data.roomCode) {
      emitError(socket, { code: "NOT_IN_ROOM", messageHe: HEBREW_ERRORS.NOT_IN_ROOM });
      return;
    }
    ...
    // socket.data.playerId (Phase 6, LIVE-04), matching every other
    // mutating handler's identity rule.
    const result = room.skipRound(data.playerId);
    if (!result.ok) {
      emitError(socket, { code: result.error, messageHe: HEBREW_ERRORS[result.error] });
      return;
    }
