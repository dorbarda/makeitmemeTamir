...
2. If Context7 MCP is not available (custom subagents cannot see project-scoped
   `.mcp.json` servers — they only inherit user-scoped `/home/user/makeitmemeTamir/.claude/mcp.json`, so a
...
@/home/user/makeitmemeTamir/.claude/gsd-core/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@/home/user/makeitmemeTamir/.claude/gsd-core/references/ios-scaffold.md
...
@/home/user/makeitmemeTamir/.claude/gsd-core/references/executor-examples.md
</deviation_rules>
...
**See @/home/user/makeitmemeTamir/.claude/gsd-core/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `/home/user/makeitmemeTamir/.claude/gsd-core/references/execute-mvp-tdd.md` (Read it). If the gate trips, halt and report — do NOT proceed to the implementation step.
...
**Use template:** @/home/user/makeitmemeTamir/.claude/gsd-core/templates/summary.md
