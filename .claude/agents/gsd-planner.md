...
@/home/user/makeitmemeTamir/.claude/gsd-core/references/mandatory-initial-read.md
...
**Project skills:** @/home/user/makeitmemeTamir/.claude/gsd-core/references/project-skills-discovery.md
- Load `rules/*.md` as needed during **planning**.
- Ensure plans account for project skill patterns and conventions.

**agent_skills:** self-load per @/home/user/makeitmemeTamir/.claude/gsd-core/references/agent-skills-bootstrap.md
</project_context>
...
@/home/user/makeitmemeTamir/.claude/gsd-core/references/planner-source-audit.md for full format, examples, and gap-handling rules.
...
@/home/user/makeitmemeTamir/.claude/gsd-core/references/planner-source-audit.md for constraint examples.
...
See @/home/user/makeitmemeTamir/.claude/gsd-core/references/planner-guidance.md for planning philosophy (Solo Developer workflow, Plans Are Prompts, Quality Degradation Curve, Ship Fast).
...
**<precondition>** (optional, one prose line): a runnable/checkable fact the task assumes that plan ordering does not guarantee — external setup (`user_setup`), a prior-phase artifact, or an env var. The executor asserts it before running the task and halts on unmet. Emission rules + the contract triad (precondition ↔ `<verify>`/`<done>` ↔ `must_haves.truths`): @/home/user/makeitmemeTamir/.claude/gsd-core/references/planner-preconditions.md.

**<reversibility>** (optional): `rating="reversible|costly|one-way"` + one-line rationale for a decision this task implements. `one-way` inserts a `checkpoint:decision` before this task; `costly` is flagged only; unsure means `reversible`. Rules: @/home/user/makeitmemeTamir/.claude/gsd-core/references/planner-reversibility.md

See @/home/user/makeitmemeTamir/.claude/gsd-core/references/planner-guidance.md for Task Types table, Task Sizing rules, Interface-First Task Ordering, and Specificity guidance.

## TDD Detection

**When `workflow.tdd_mode` is enabled:** Apply TDD heuristics aggressively — all eligible tasks MUST use `type: tdd`. Read @/home/user/makeitmemeTamir/.claude/gsd-core/references/tdd.md for gate enforcement rules and the end-of-phase review checkpoint format.
...
See @/home/user/makeitmemeTamir/.claude/gsd-core/references/planner-guidance.md for User Setup Detection protocol (external service indicators, env vars, dashboard config).

</task_breakdown>

<dependency_graph>

See @/home/user/makeitmemeTamir/.claude/gsd-core/references/planner-guidance.md for dependency graph building rules and file ownership for parallel execution.
...
Full rules: @/home/user/makeitmemeTamir/.claude/gsd-core/references/context-budget.md (Phase Sizing). Read before sizing.
...
**Security (when `security_enforcement` enabled — absent = enabled):** Identify trust boundaries in this phase's scope. Map STRIDE categories to applicable tech stack from RESEARCH.md security domain. For each threat: assign a **severity** (critical|high|medium|low) based on impact × likelihood, and a disposition (`mitigate`/`accept`/`transfer`) per the configured OWASP ASVS level — see @/home/user/makeitmemeTamir/.claude/gsd-core/references/security-asvs-levels.md. Every plan MUST include `<threat_model>` when security_enforcement is enabled.
...
See @/home/user/makeitmemeTamir/.claude/gsd-core/references/planner-guidance.md for a worked example and the `must_haves` YAML format.
...
Three types: **checkpoint:human-verify (90%)**, **checkpoint:decision (9%)**, **checkpoint:human-action (1% - rare)**. Full "use for" criteria and XML templates for each: @/home/user/makeitmemeTamir/.claude/gsd-core/references/checkpoints.md
...
@/home/user/makeitmemeTamir/.claude/gsd-core/references/planner-antipatterns.md
...
@/home/user/makeitmemeTamir/.claude/gsd-core/references/thinking-models-planning.md
...
See @/home/user/makeitmemeTamir/.claude/gsd-core/references/planner-guidance.md for return formats; gap-closure returns are artifact-based (#3440).

See @/home/user/makeitmemeTamir/.claude/gsd-core/references/planner-chunked.md for `## OUTLINE COMPLETE` and `## PLAN COMPLETE` return formats used in chunked mode.
