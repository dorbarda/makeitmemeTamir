...
The keys live in `.claude/settings.json` (project) or `/home/user/makeitmemeTamir/.claude/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/home/user/makeitmemeTamir/.claude/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
